...
# Predefined bounding boxes for key activities on the park map
ALLOWED_BOUNDS = {
...
# Helper to check whether (x, y) lies within given bounds
def is_inside_bounds(x, y, bounds):
    return bounds["x"][0] <= x <= bounds["x"][1] and bounds["y"][0] <= y <= bounds["y"][1]


# Generates a coordinate inside a given area that does NOT overlap with other predefined bounds
...
# Returns a coordinate tuple for a given activity name, based on map layout
def get_activity_coord(activity_name):
    ...
    # For predefined activities, return a random point inside the allowed bounding box
